...
1. **Frontend-First Strategy (Phase 1: FE Demo $\to$ Phase 2: BE Integration):**
   * **Phase 1 (Current Active Scope):** 100% interactive Frontend using Next.js 16, React 19, Tailwind CSS, and Zustand 5. All data (scenario, vessel particulars, documents, timeline events, and scoring formulas) is encapsulated in typed frontend fixtures. The complete cadet learning loop (7 screens) operates with zero backend dependency.
   * **Phase 2 (Subsequent Scope - Post-FE Approval):** Once the frontend UX, simulation fidelity, and client presentation are validated and approved, the backend layer (API routes, database persistence for cadet session history, instructor telemetry feeds) will be attached.
2. **Deterministic Scripted Simulation Engine:** Physics simulation is strictly out-of-scope (PRD Section 17 & 32). The simulation runs on a normalized temporal clock ($T+00$ to $T+45$), interpolating positions for ship berthing, quay crane cycles, and container truck transfers.
3. **Deterministic Pedagogical State Machine:** The entire user journey is governed by a finite state machine (FSM) transitioning sequentially across 9 discrete phases.
...
│   └── TECHNICAL_SPEC.md            # This technical design document
├── public/
│   └── assets/
